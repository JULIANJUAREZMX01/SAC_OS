...
//var _xmlhttp = factoryXmlHttpRequest();
...
function geturl(url)
{
	...
	if (_xmlhttp)
	{
		_xmlhttp.open("GET", urlex(url), false);
		try {
			_xmlhttp.send(null);
...
function posturl(url, xml)
{
	...
	if (_xmlhttp)
	{
		_xmlhttp.open("POST", urlex(url), false);
		_xmlhttp.setRequestHeader("Content-Type", "text/xml");
		_xmlhttp.setRequestHeader("Content-Length", xml.length);
...
{
	var retval = null;
	if (xml == null) { alert("puturl: null data."); return; }
	if (xml.length == 0) { alert("puturl: zero length data."); return; }
	var _xmlhttp = factoryXmlHttpRequest();
	if (_xmlhttp)
	{
		_xmlhttp.open("PUT", urlex(url), false);
		_xmlhttp.setRequestHeader("Content-Type", "text/xml");
		_xmlhttp.setRequestHeader("Content-Length", xml.length);
		_xmlhttp.setRequestHeader("Connection", "close");
		//_xmlhttp.onreadystatechange = function() {}
...
function deleteurl(url)
{
	...
	if (_xmlhttp)
	{
		_xmlhttp.open("DELETE", urlex(url), false);
		_xmlhttp.setRequestHeader("Content-Type", "text/xml");
		//_xmlhttp.setRequestHeader("Content-Length", xml.length);
		_xmlhttp.setRequestHeader("Connection", "close");
		//_xmlhttp.onreadystatechange = function() {}
...
function ping()
{
	...
	if (_xmlhttp)
	{
		_xmlhttp.open("GET", urlex("ping.xml?ping"), false);
		try {
			_xmlhttp.send(null);
			return true;
		}
